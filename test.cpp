...
TEST(GetSubTrie, empty_trie) {
	Trie <int> a;
	std::string my_str("empty");
	SubTrie<int> b=a.GetSubTrie(my_str);
	EXPECT_EQ(a.size(), 0);
	EXPECT_TRUE(a.empty());
	std::pair < const std::string, int> cur = std::make_pair(std::string(), int());
	EXPECT_EQ(b.value(),cur );

}
TEST(GetSubTrie, empty_trie_empty_str) {
	Trie <int> a;
	std::string my_str;
	SubTrie<int> b = a.GetSubTrie(my_str);
	EXPECT_EQ(a.size(), 0);
...
TEST(GetSubTrie, one_element_trie) {
	Trie <int> a;
	std::string my_str("first");
	a.insert(my_str, 54);
	SubTrie<int> b = a.GetSubTrie(my_str);
	EXPECT_EQ(a.size(), 1);
	std::pair < const std::string, int> cur = std::make_pair(my_str,54);
	EXPECT_EQ(b.value(), cur);
}
TEST(GetSubTrie, one_element_trie_empty_str) {
	Trie <int> a;
	std::string my_str("first");
	a.insert(my_str, 54);
	SubTrie<int> b = a.GetSubTrie(std::string());
	EXPECT_EQ(a.size(), 1);
	std::pair < const std::string, int> cur = std::make_pair(std::string(), int());
	EXPECT_EQ(b.value(), cur);
}
TEST(GetSubTrie, one_element_trie_alien_str) {
	Trie <int> a;
	std::string my_str("first");
	a.insert(my_str, 54);
	SubTrie<int> b = a.GetSubTrie("two");
	EXPECT_EQ(a.size(), 1);
	std::pair < const std::string, int> cur = std::make_pair(std::string(), int());
	EXPECT_EQ(b.value(), cur);
}
TEST(GetSubTrie, two_element_trie) {
	Trie <int> a;
	std::string my_str("first");
	a.insert(my_str, 54);
	std::string my_str_2 = "two";
	a.insert(my_str_2, 76);
	SubTrie<int> b = a.GetSubTrie(my_str);
	SubTrie<int> c = a.GetSubTrie(my_str_2);
	EXPECT_EQ(a.size(), 2);
	std::pair < const std::string, int> cur = std::make_pair(my_str, 54);
	std::pair < const std::string, int> cur_2 = std::make_pair(my_str_2, 76);
	EXPECT_EQ(c.value(),cur_2);
	EXPECT_EQ(b.value(), cur);
}
...
TEST(GetSubTrie, GetSubTrie) {
	std::string my_str = "hgoreihgrf";
	std::vector<std::pair<const std::string, int>> my_vector;
	Trie <int> a;
	for (size_t i = 0; i < my_str.size(); ++i) {
		a.insert(my_str.substr(i), i);
		my_vector.push_back(std::make_pair(my_str.substr(i), i));
	}
	size_t counter = 0;
	EXPECT_EQ(a.size(), my_str.size());
	for (size_t i = 0; i < my_str.size(); ++i) {
		SubTrie<int> b = a.GetSubTrie(my_str.substr(i));
		if(b.value()==my_vector[i])
			++counter;
	}
...
TEST(GetSubTrie, GetSubTrie_str) {
	std::string my_str = "hgoreihgrf";
	std::vector<std::pair<const std::string, std::string >> my_vector;
	Trie <std::string > a;
	for (size_t i = 0; i < my_str.size(); ++i) {
		a.insert(my_str.substr(i), my_str.substr(i));
		my_vector.push_back(std::make_pair(my_str.substr(i), my_str.substr(i)));
	}
	size_t counter = 0;
	EXPECT_EQ(a.size(), my_str.size());
	for (size_t i = 0; i < my_str.size(); ++i) {
		SubTrie<std::string> b = a.GetSubTrie(my_str.substr(i));
		if (b.value() == my_vector[i])
			++counter;
	}
	EXPECT_EQ(counter, my_str.size());
...
TEST(GetSubTrie, many_elements) {
	std::string my_str = "hgoreihgrfdfjbjgogghndgd";
	std::vector<std::pair<const std::string, int>> my_vector;
	Trie <int> a;
	for (size_t i = 0; i < my_str.size(); ++i) {
		a.insert(my_str.substr(i), i);
...
TEST(GetSubTrie, many_elements_str) {
	std::string my_str = "hgoreihgrf'kb;t;gfgrhfgng";
	std::vector<std::pair<const std::string, std::string >> my_vector;
	Trie <std::string > a;
	for (size_t i = 0; i < my_str.size(); ++i) {
		a.insert(my_str.substr(i), my_str.substr(i));
...
TEST(const_iterator, one_el_trie) {
	Trie <int> a;
	std::string my_str("first");
	 a.insert(my_str, 54);
	auto [key, value] = *(a.cbegin());
	EXPECT_EQ(key, my_str);
	EXPECT_EQ(value, 54);
}
...
TEST(const_iterator, empty_trie) {
	Trie<int> a;
...
TEST(const_iterator, ten_elements)
{
	std::string my_str = "hgtyruejnf";
	...
	const Trie<int> a(b.begin(), b.end());
	EXPECT_EQ(a.size(), my_str.size());
	size_t counter = 0;
	for (size_t i = 0; i < 10; ++i) {
		auto& [key, value] = *a.find(my_str.substr(i));
...
TEST(const_iterator, ten_elements_alien)
{
	std::string my_str = "hgtyruejnf";
	std::vector<std::pair<std::string, int>>  b;
	for (size_t i = 0; i < my_str.size(); ++i) {
	...
	const Trie<int> a(b.begin(), b.end());
	EXPECT_EQ(a.size(), my_str.size());
	EXPECT_EQ(a.find("lol"),a.cend());
}
TEST(const_iterator, find_ten_elements)
{
	std::string my_str = "hgtyruejnf";
	std::vector<std::pair<std::string, int>>  b;
	for (size_t i = 0; i < my_str.size(); ++i) {
...
TEST(const_iterator, find_alien_elements)
{
	std::string my_str = "hgtyruejnf";
	std::vector<std::pair<std::string, int>>  b;
	for (size_t i = 0; i < my_str.size(); ++i) {
...
TEST(GetSubTrie, 100_elements) {
	std::string my_str = "node   ";
	std::vector<std::pair<const std::string, int>> my_vector;
	Trie <int> a;
	for (size_t i = 0; i < 100; ++i) {
		a.insert(my_str.replace(5, 3, std::to_string(i)), i);
		my_vector.push_back(std::make_pair(my_str.replace(5, 3, std::to_string(i)), i));
	}
	size_t counter = 0;
	EXPECT_EQ(a.size(),100);
	for (size_t i = 0; i < 100; ++i) {
		SubTrie<int> b = a.GetSubTrie(my_str.replace(5, 3, std::to_string(i)));
		if (b.value() == my_vector[i])
			++counter;
	}
...
TEST(erase_iterator, 100_elements)
{
	std::string my_str = "node  ";
	Trie <int> a;
	for (size_t i = 0; i <100; ++i) {
...
TEST(const_iterator, find_one)
{
	std::vector< std::pair<std::string, int>>a;
	a.push_back(std::make_pair("lol", 4));
	const Trie<int> k(a.begin(),a.end());
	Trie<int>::const_iterator it = k.find("lol");
...
TEST(const_iterator, insert_100_assign_iterate_check)
{
	Trie<int> k;
	...
	auto it = k.cbegin();
	auto amount = 0;
	for (auto it1 = k.cbegin(); it1 != k.cend();++ it1) {
		EXPECT_EQ((*it1).first, (*it).first);
		++it;
		++amount;
	}
...
TEST(const_iterator, insert_10_iterate)
{
	Trie<int> k;
